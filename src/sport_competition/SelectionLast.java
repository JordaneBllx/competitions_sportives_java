...
/**
 * SelectionLast, a type of SelectionStrategy
 * All the first in each pool end up in the terminal phase.If the number of players is not a power of two (necessary for the execution of a tournament), the best (or all) of the last ones are drafted, then the best second ones are also drafted if necessary.
 * @see SelectionStrategy
 * @version 2.0
 */
public class SelectionLast implements SelectionStrategy  { 
    /**
    ...
    public ArrayList<Competitor> getCompetitorsForFinalTournament (ArrayList<League> leagues) {
        ...
        for (Map<Competitor,Integer> rank : listRanking){
          /*for (Map.Entry<Competitor,Integer> entry : rank.entrySet()) {
            ...
            while (it.hasNext())
                cLast = (Competitor)((Map.Entry)it.next()).getKey();
            listNumberLast.add(cLast);
        }
        Map<Competitor,Integer> rank2 = (new League(new ClassicalMatch(), listNumber2)).ranking();
        Map<Competitor,Integer> rankLast = (new League(new ClassicalMatch(), listNumberLast)).ranking();
        int listSize1 = listNumber1.size() ;
        int listSize2 = listNumber2.size() ;
        int listSizeLast = listNumberLast.size() ;
        int j=0;
        int i=0;
        Iterator it2 = rank2.entrySet().iterator();
        Iterator itLast = rankLast.entrySet().iterator();
        while (!SelectionStrategy.powerOfTwoWithoutException(listSize1+i) && (i+j)<listSizeLast+listSize2) {
            if (i<listSizeLast) {
                listNumber1.add((Competitor)((Map.Entry)itLast.next()).getKey());
                i++;
            }
            else {
